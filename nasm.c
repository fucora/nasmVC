...
(4)使用文档和代码间的注释相当齐全，便于他人阅读。
  ...
  同时，NASM也存在以下问题：
  (1)各模块间无法进行相互间的通信。这就导致模块间存在一些重复性代码。另外在模块内部也有少许重复代码。
  (2)listing.c模块和eval.c模块功能简单，特别是listing.c目前尚不完善。
  (3)实质上的架构只提供了两遍的源码扫描，这就导致它对表达式的要求比较严格，不能处理需要多遍扫描才能
  　 完成的表达式，这方面的内容可参阅NASM的帮助文档中的Critical Expressions一节。
...
/*
 * 下面两个变量都在report_error函数中修改。
 */
//是否print使用说明 type `nasm -h' for help\n
