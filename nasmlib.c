...
*error---错误与否
*返回值：读的结果
*注解：（1）在NASM中，如果数以0x,0X,$开头，H，h,结尾为十六进制。如果以Q结尾为八进
*制，B结尾为二进制。
*（2）checklimit为检查的限制，也就是说，如果现在result>=checklist，那么，假如当前
*digit
*则，
*        result=result*radix+digit>2**32,不能用32位长的空间表示。
*  而last就是当result == checklimit 时digit 可能的最大值last。
*  表示为  last=2**32-[checklist]*radix.例如radix=10,last=2**32-[2**32/10]*10=6;
*/
...
/*
*把字符串当数字存储。例如”1234”存储成(4+’0’)*(2**24)+ (3+’0’)*(2**16)+(‘2’
*+0’) *(2**8)+(1+’0’)
*/
...
//标准化扩展名。Inname为输入的名字，outname为输出的名字
...
struct SAA *saa_init (long elem_len) 
{
    ...
    s->posn = s->start = 0L;
    s->elem_len = elem_len;
    s->length = SAA_MAXLEN - (SAA_MAXLEN % elem_len);//分配的大小是elem_len的倍数
...
*/
#include "names.c"
//存放条件指令，例如"a16", "a32", "lock", "o16", "o32", "rep", "repe", "repne"
...
/*把p指向的，长度为len的数据存放在stds
*can_tempstorage（存放数据串的缓冲区指针定义为static char **stdscan_tempstorage =
*NULL;）中
*/
...
//读取token,private_data为当前的输入指针，tv存放结果。返回类型
...
*/
//如果是可重定位的，返回TRUE包括简单标量，加上段基址，和WRT
...
*/
//返回可重定位的WRT的段号。在16位程序中，wrt用于指定计算的segment，例如
// mov ax,weird_seg       ; weird_seg is a segment base 
// mov es,ax 
// mov bx,symbol wrt weird_seg
//bx里存放的是相对于weird_seg的段偏移，而不是相对于缺省的段，如DS的偏移。
